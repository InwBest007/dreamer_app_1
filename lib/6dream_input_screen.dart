// DreamInputScreen.dart (เพิ่มการตรวจเช็คการแก้คำผิดโดย user)
import 'dart:convert';
...
class _DreamInputScreenState extends State<DreamInputScreen> {
  ...
  Future<void> _handleSubmit() async {
    ...
    try {
      // เรียก analyze endpoint เพื่อขอ corrected tokens (ใช้สำหรับเสนอเวอร์ชันที่แก้ให้ผู้ใช้)
      ...
      try {
        ...
        if (resp.statusCode == 200) {
          ...
          // ฝั่ง server คืน "tokens" หรือ "tokens" และ "matched_keywords"
          // ใน dream_api.py คุณคืน corrected_tokens เป็น "tokens" ชื่อเดิม (แก้ typo)
          // ถ้า server คืน corrected tokens ใน key 'tokens' หรือ 'corrected_tokens'
          ...
          if (correctedTokens != null && correctedTokens.isNotEmpty) {
            // สำหรับภาษาไทย ให้ต่อคำโดยไม่มีช่องว่าง
            final candidate = correctedTokens.map((e) => e.toString()).join('');
        ...
        // ถ้าเรียก analyze ไม่ได้ ไม่ต้องบล็อกผู้ใช้ — จะ fallback ไปใช้ต้นฉบับ
