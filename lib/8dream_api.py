...
# โหลด custom dict และขยายเป็น bidirectional mapping
...
# โหลด thai_synonym จาก pythainlp จาก GitHub อย่างเดียวเท่านั้น
    try:
...
async def analyze_dream(req: DreamRequest):
    try:
        ...
        # 2. แก้ typo (ใช้ thaispellcheck) — แก้ทั้งประโยคก่อน แล้วตัดคำ
        try:
            corrected_text = await asyncio.to_thread(thaispellcheck.check, dream_text, autocorrect=True)
            ...
            print("[DEBUG] Spellcheck failed, fallback to original tokens:", e)
            corrected_text = dream_text
            corrected_tokens = word_tokenize(dream_text, engine="newmm")
